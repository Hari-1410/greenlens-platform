...
export async function POST(req: NextRequest) {
  try {
    // Try session auth first, fall back to userId in body (extension)
    ...
    if (data.externalId) {
      ...
      if (existing) {
        return NextResponse.json({ error: "Duplicate purchase", purchase: existing }, { status: 409 });
      }
    }
    ...
    // Create purchase + update wallet + record transaction in a transaction
    const [purchase] = await prisma.$transaction([
      ...
      prisma.transaction.create({
        data: {
          ...
          tokens,
          moneyValue,
          description: `Eco purchase: ${data.productName.slice(0, 50)}`,
        },
      }),
    ]);
